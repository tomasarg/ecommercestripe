<template>
  <div class="checkout-box">
    ...
    <h3 class="total" v-if="hasProduct()">
      Total: $ {{ totalPrice() }}, 00
      <div style="text-align:right;margin-top: 50px;"><button id="checkout-button" @click="pay()">Pay</button></div>
    </h3>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async pay(index) {
      ...
      const response = await fetch(config.BACKEND.API_URL + 'checkout-session', {
        method: 'POST',
        headers: {
